...
def main():
    ...
    ps = ArgumentParser(description="Download NVSS stamps.")
    ...
    if args.name is None:
        args.name = "J{:.2f}{:+.2f}_nvss".format(coords[0], coords[1])
        print("Setting name to {}".format(args.name))

    postage_stamps.NVSS(coords[0], coords[1], args.fov, args.directory, args.name,
